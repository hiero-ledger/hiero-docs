...
Queries are requests processed only by the single node to which they are sent. [Clients](broken-reference) send queries to retrieve some aspect of the current consensus state, like the balance of an account. Certain queries are free, but generally, queries are subject to fees.
