...
A key can be a [public key](broken-reference) of a supported system [Ed25519](broken-reference), [ECDSA secp256k1](broken-reference), or an ID of a [smart contract](broken-reference). The corresponding algorithm generates public and private keys which are unique to one another. The public key can be shared and visible to other network users in a [Network Explorer](broken-reference) or [REST APIs](broken-reference). The [private key](broken-reference) is kept secret from the owner and grants access to the owner to modify entities (accounts, tokens, etc.).&#x20;
...
A key in Hedera can be a [public key](broken-reference) of a supported system such as [ED25519](broken-reference), [ECDSA secp256k1](broken-reference), or an ID of a [smart contract](broken-reference). The corresponding algorithm generates public and private keys which are unique to one another. The public key can be shared and visible to other network users in a [Network Explorer](broken-reference) or REST APIs. The [private key](broken-reference) is kept secret and grants access to the owner to modify entities (accounts, tokens, etc.).
